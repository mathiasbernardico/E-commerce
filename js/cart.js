...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const radioInputs = document.querySelectorAll(
    '#contenedorDeEnvios input[type="radio"]'
  );
  ...
  comprarButton.addEventListener("click", function (event) {
    ...
    const formaDeEnvioInputs = document.querySelectorAll(
      'input[name="opcion"]'
    );
    const cantidadInputs = document.querySelectorAll(
      'input[name="inputQuantity"]'
    );
    ...
    if (
      ...
      isNaN(numeroInput.value) ||
      parseInt(numeroInput.value) < 0
    ) {
    ...
    if (formaDePagoSeleccionada) {
      const selectedFormaDePago = document.querySelector(
        'input[name="op"]:checked'
      ).value;

      if (selectedFormaDePago === "credito") {
        if (
          numeroTarjetaInput.value.trim() === "" ||
          codigoInput.value.trim() === "" ||
          vencimientoInput.value.trim() === ""
        ) {
          showErrorMessage(
            "tarjetaError",
            "Los campos de tarjeta no pueden estar vacíos"
          );
          allValid = false;
        } else {
    ...
    if (allValid) {
      ...
      if (snackbar) {
        window.scrollTo({
          top: snackbar.offsetTop - window.innerHeight / 2,
          behavior: "smooth",
        });
        snackbar.style.display = "block";
      }
