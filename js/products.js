...
function productos(listaDeProductos, textobuscado) {
  ...
  for (let producto of listaDeProductos) { //Recorremos la listaDeProductos buscando en cada producto
    if (producto.name.toLowerCase().includes(textobuscado) || producto.description.toLowerCase().includes(textobuscado)){ //Se evalua que el nombre o la descripcion incluya el texto buscado (el texto lo comparamos transformandolo todo a minuscula)
      ...
      divProductContainer.addEventListener("click", () => { //Agregamos el evento click en el div anteriormente creado
...
function clasificacion_products(dato) { //Creamos la funcion para clasificar el producto en funcion del dato
  fetch(URLProducts)
    ...
    .then((data) => { 
      ...
      if (dato == "0") {
        data.products.sort(function (a, b) { //Clasificacion acendente (Precio)
          return a.cost - b.cost;
        });
      } else if (dato == "1") {
        data.products.sort(function (a, b) { //Clasificacion decendente (Precio)
          return b.cost - a.cost;
        });
      } else if (dato == "2") {
        data.products.sort(function (a, b) { //Clasificacion decendente (Relevancia)
...
function categoria(categoria) { //Colocamos en el parrafo el nombre de la categoria 
...
busqueda.addEventListener("input", function () { //Si se utiliza el input de busqueda
...
function filtrarMinMax(lista) {
  if(inputMaximo.value !== "" && inputMinimo.value !== ""){ //En el caso que rellene los dos campos, controlamos que los dos no esten vacios
    return lista.filter((item) => { //Utilizamos .filter para filtrar y utilizamos una funcion flecha para aplicar la siguiente condicion
      return item.cost >= inputMinimo.value && item.cost <= inputMaximo.value;
    });
  }else if(inputMaximo.value !== ""){ //En caso que solo rellene el inputMaximo
    return lista.filter((item) => {
      return item.cost <= inputMaximo.value;
    });
  }else if(inputMinimo.value !== ""){ //En caso que solo rellene el inputMinimo
    return lista.filter((item) => {
      return item.cost >= inputMinimo.value;
    });
  }
  }
  botonFiltrarRangos.addEventListener("click", () => { //Evento click para el boton filtrar
  if (inputMaximo.value !== "" || inputMinimo.value !== "") { //Controlamos que no esten vacios
    fetch(URLProducts)
      .then((response) => response.json())
      .then((data) => {
